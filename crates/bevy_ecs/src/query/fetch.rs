...
/// SAFETY:
/// `update_component_access` and `update_archetypes` do nothing.
...
/// This is sound because `update_component_access` and `update_archetypes` set read access for all components and panic when appropriate.
...
/// This is sound because `update_component_access` and `update_archetypes` add read access for that component and panic when appropriate.
...
/// This is sound because `update_component_access` and `update_archetypes` add write access for that component and panic when appropriate.
/// `update_component_access` adds a `With` filter for a component.
/// This is sound because `matches_component_set` returns whether the set contains that component.
...
/// This is sound because `update_component_access` and `update_archetypes` add the same accesses as `T`.
/// Filters are unchanged.
...
macro_rules! impl_anytuple_fetch {
    ($(#[$meta:meta])* $(($name: ident, $state: ident)),*) => {
        ...
        /// This is sound because `update_component_access` and `update_archetypes` adds accesses according to the implementations of all the subqueries.
