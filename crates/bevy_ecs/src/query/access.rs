...
impl Access {
    ...
    pub const fn new() -> Self {
        Self {
            ...
            read_and_writes: ComponentSet::new(),
            writes: ComponentSet::new(),
            archetypal: ComponentSet::new(),
        }
    }
    ...
    pub fn get_conflicts(&self, other: &Access) -> AccessConflicts {
        let mut conflicts = ComponentSet::new();
        ...
        for (
            ...
            let temp_conflicts: ComponentSet =
                match (lhs_writes_inverted, rhs_reads_and_writes_inverted) {
    ...
    ///
    /// [`Has<T>`]: crate::query::Has
    pub fn archetypal(&self) -> &ComponentSet {
        &self.archetypal
    }

    /// Returns the set of components with read or write access,
    /// or an error if the access is unbounded.
    pub fn try_reads_and_writes(&self) -> Result<&ComponentSet, UnboundedAccessError> {
    ...
    /// or an error if the access is unbounded.
    pub fn try_writes(&self) -> Result<&ComponentSet, UnboundedAccessError> {
        if self.writes_inverted {
            return Err(UnboundedAccessError {
                writes_inverted: self.writes_inverted,
                read_and_writes_inverted: self.read_and_writes_inverted,
...
fn invertible_union_with(
    self_set: &mut ComponentSet,
    self_inverted: &mut bool,
    other_set: &ComponentSet,
    other_inverted: bool,
) {
...
pub struct FilteredAccess {
    ...
    pub(crate) required: ComponentSet,
...
impl AccessConflicts {
    ...
    pub(crate) fn empty() -> Self {
        Self::Individual(ComponentSet::new())
    }
}
...
impl FilteredAccess {
    ...
    pub fn required(&self) -> &ComponentSet {
        &self.required
    }
...
pub struct AccessFilters {
    pub(crate) with: ComponentSet,
    pub(crate) without: ComponentSet,
}
...
pub struct ComponentSet(FixedBitSet);

impl ComponentSet {
    /// Create a new empty `ComponentSet`.
    #[inline]
    pub const fn new() -> Self {
    ...
    /// Returns a lazy iterator over the union of two [`ComponentSet`]s.
    #[inline]
    pub fn union<'a>(&'a self, other: &'a ComponentSet) -> impl Iterator<Item = ComponentId> + 'a {
        self.0.union(&other.0).map(ComponentId::new)
    }

    /// Returns a lazy iterator over the intersection of two [`ComponentSet`]s.
    #[inline]
    pub fn intersection<'a>(
        &'a self,
        other: &'a ComponentSet,
    ) -> impl Iterator<Item = ComponentId> + 'a {
        self.0.intersection(&other.0).map(ComponentId::new)
    }

    /// Returns a lazy iterator over the difference of two [`ComponentSet`]s.
    #[inline]
    pub fn difference<'a>(
        &'a self,
        other: &'a ComponentSet,
    ...
    /// In-place union of two [`ComponentSet`]s.
    #[inline]
    pub fn union_with(&mut self, other: &ComponentSet) {
        self.0.union_with(&other.0);
    }

    /// In-place intersection of two [`ComponentSet`]s.
    #[inline]
    pub fn intersect_with(&mut self, other: &ComponentSet) {
        self.0.intersect_with(&other.0);
    }

    /// In-place difference of two [`ComponentSet`]s.
    #[inline]
    pub fn difference_with(&mut self, other: &ComponentSet) {
        self.0.difference_with(&other.0);
    }

    /// In-place reversed difference of two [`ComponentSet`]s.
    /// This sets `self` to be `other.difference(self)`.
    #[inline]
    pub fn difference_from(&mut self, other: &ComponentSet) {
...
mod tests {
    ...
    use crate::{
        ...
        query::{
            access::AccessFilters, Access, AccessConflicts, ComponentAccessKind, ComponentSet,
    ...
    fn filtered_access_extend_or() {
        ...
        expected.filter_sets = vec![
            AccessFilters {
                with: ComponentSet::from_bits(FixedBitSet::with_capacity_and_blocks(3, [0b111])),
                without: ComponentSet::default(),
            },
            AccessFilters {
                with: ComponentSet::from_bits(FixedBitSet::with_capacity_and_blocks(4, [0b1011])),
                without: ComponentSet::from_bits(FixedBitSet::with_capacity_and_blocks(
                    5,
                    [0b10000],
                )),
            },
    ...
    /// Create a `ComponentSet` with a given number of total bits and a given list of bits to set.
    /// Setting the number of bits is important in tests since the `PartialEq` impl checks that the length matches.
    fn bit_set(bits: usize, iter: impl IntoIterator<Item = usize>) -> ComponentSet {
        let mut result = FixedBitSet::with_capacity(bits);
        result.extend(iter);
        ComponentSet::from_bits(result)
    }
