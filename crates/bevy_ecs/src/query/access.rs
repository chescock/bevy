...
impl InvertibleSet {
    ...
    /// This is equivalent to calling `insert()` on each value in `bits`,
    /// followed by a call to `invert()` if `inverted` is `true`.
    #[cfg(test)]
    pub fn from_bits_and_inverted(inverted: bool, bits: impl IntoIterator<Item = usize>) -> Self {
        Self {
    ...
    pub fn union_with(&mut self, other: &Self) {
        match (self.inverted, other.inverted) {
            (false, false) => self.set.union_with(&other.set),
            (false, true) => {
                self.inverted = true;
            ...
            (true, false) => self.set.difference_with(&other.set),
            (true, true) => self.set.intersect_with(&other.set),
        }
    }
...
mod tests {
    ...
    fn invertible_union_with_tests() {
        ...
        // [0, 1] | [0, 2] = [0, 1, 2]
        let mut set = InvertibleSet::from_bits_and_inverted(false, [0, 1]);
        set.union_with(&InvertibleSet::from_bits_and_inverted(false, [0, 2]));
        assert_eq!(set, InvertibleSet::from_bits_and_inverted(false, [0, 1, 2]));

        // [0, 1] | [1, 3, ...] = [0, 1, 3, ...]
        let mut set = InvertibleSet::from_bits_and_inverted(false, [0, 1]);
        set.union_with(&InvertibleSet::from_bits_and_inverted(true, [0, 2]));
        assert_eq!(set, InvertibleSet::from_bits_and_inverted(true, [2]));

        // [2, 3, ...] | [0, 2] = [0, 2, 3, ...]
        let mut set = InvertibleSet::from_bits_and_inverted(true, [0, 1]);
        set.union_with(&InvertibleSet::from_bits_and_inverted(false, [0, 2]));
        assert_eq!(set, InvertibleSet::from_bits_and_inverted(true, [1]));

        // [2, 3, ...] | [1, 3, ...] = [1, 2, 3, ...]
        let mut set = InvertibleSet::from_bits_and_inverted(true, [0, 1]);
        set.union_with(&InvertibleSet::from_bits_and_inverted(true, [0, 2]));
        assert_eq!(set, InvertibleSet::from_bits_and_inverted(true, [0]));
    }
    ...
    fn invertible_union_with_different_lengths() {
        ...
        // [0] | [2, ...] = [0, 2, ...]
        let mut set = InvertibleSet::from_bits_and_inverted(false, [0]);
        set.union_with(&InvertibleSet::from_bits_and_inverted(true, [0, 1]));
        assert_eq!(set, InvertibleSet::from_bits_and_inverted(true, [1]));
    }
    ...
    fn invertible_difference_with_tests() {
        ...
        // [0, 1] - [0, 2] = [1]
        let mut set = InvertibleSet::from_bits_and_inverted(false, [0, 1]);
        set.difference_with(&InvertibleSet::from_bits_and_inverted(false, [0, 2]));
        assert_eq!(set, InvertibleSet::from_bits_and_inverted(false, [1]));

        // [0, 1] - [1, 3, ...] = [0]
        let mut set = InvertibleSet::from_bits_and_inverted(false, [0, 1]);
        set.difference_with(&InvertibleSet::from_bits_and_inverted(true, [0, 2]));
        assert_eq!(set, InvertibleSet::from_bits_and_inverted(false, [0]));

        // [2, 3, ...] - [0, 2] = [3, ...]
        let mut set = InvertibleSet::from_bits_and_inverted(true, [0, 1]);
        set.difference_with(&InvertibleSet::from_bits_and_inverted(false, [0, 2]));
        assert_eq!(set, InvertibleSet::from_bits_and_inverted(true, [0, 1, 2]));

        // [2, 3, ...] - [1, 3, ...] = [2]
        let mut set = InvertibleSet::from_bits_and_inverted(true, [0, 1]);
        set.difference_with(&InvertibleSet::from_bits_and_inverted(true, [0, 2]));
        assert_eq!(set, InvertibleSet::from_bits_and_inverted(false, [2]));
    }
    ...
    fn invertible_intersect_with_tests() {
        ...
        // [0, 1] & [0, 2] = [0]
        let mut set = InvertibleSet::from_bits_and_inverted(false, [0, 1]);
        set.intersect_with(&InvertibleSet::from_bits_and_inverted(false, [0, 2]));
        assert_eq!(set, InvertibleSet::from_bits_and_inverted(false, [0]));

        // [0, 1] & [1, 3, ...] = [1]
        let mut set = InvertibleSet::from_bits_and_inverted(false, [0, 1]);
        set.intersect_with(&InvertibleSet::from_bits_and_inverted(true, [0, 2]));
        assert_eq!(set, InvertibleSet::from_bits_and_inverted(false, [1]));
