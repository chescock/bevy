...
impl<'w> UnsafeWorldCell<'w> {
    ...
    pub(crate) unsafe fn get_many_entities_mut<T: TrustedEntityBorrow, const N: usize>(
        self,
        entities: [T; N],
    ) -> Result<UniqueEntityArray<EntityMut<'w>, N>, EntityMutableFetchError> {
    ...
    pub(crate) unsafe fn get_many_entities_unique_mut<T: TrustedEntityBorrow, const N: usize>(
    ...
    ) -> Result<UniqueEntityArray<EntityMut<'w>, N>, EntityDoesNotExistError> {
        ...
        // SAFETY: Caller ensures all entities are unique, and each `EntityMut` corresponds to one of them
        Ok(unsafe { UniqueEntityArray::from_array_unchecked(refs) })
    }
