...
impl<'w> EntityWorldMut<'w> {
    ...
    /// assert_eq!(world.query_mut::<&Comp>().single_inner().unwrap().0, 4);
    ///
    /// # let mut entity = world.get_entity_mut(entity_id).unwrap();
    /// entity.entry::<Comp>().and_modify(|mut c| c.0 += 1);
    /// assert_eq!(world.query_mut::<&Comp>().single_inner().unwrap().0, 5);
    /// ```
    ///
    /// # Panics
    ///
...
impl<'w, 'a, T: Component<Mutability = Mutable>> Entry<'w, 'a, T> {
    ...
    ///
    /// entity.entry::<Comp>().and_modify(|mut c| c.0 += 1);
    /// assert_eq!(world.query_mut::<&Comp>().single_inner().unwrap().0, 1);
    /// ```
    #[inline]
...
impl<'w, 'a, T: Component> Entry<'w, 'a, T> {
    ...
    /// assert_eq!(world.query_mut::<&Comp>().single_inner().unwrap().0, 8);
    /// ```
    #[inline]
...
impl<'w, 'a, T: Component + Default> Entry<'w, 'a, T> {
    ...
    /// assert_eq!(world.query_mut::<&Comp>().single_inner().unwrap().0, 0);
    /// ```
    #[inline]
...
impl<'w, 'a, T: Component> OccupiedEntry<'w, 'a, T> {
    ...
    /// }
    ///
    /// assert_eq!(world.query_mut::<&Comp>().single_inner().unwrap().0, 10);
    /// ```
    #[inline]
    ...
    /// }
    ///
    /// assert_eq!(world.query_mut::<&Comp>().into_iter().len(), 0);
    /// ```
    #[inline]
    pub fn take(self) -> T {
        // This shouldn't panic because if we have an OccupiedEntry the component must exist.
...
impl<'w, 'a, T: Component<Mutability = Mutable>> OccupiedEntry<'w, 'a, T> {
    ...
    /// }
    ///
    /// assert_eq!(world.query_mut::<&Comp>().single_inner().unwrap().0, 17);
    /// ```
    #[inline]
    ...
    /// }
    ///
    /// assert_eq!(world.query_mut::<&Comp>().single_inner().unwrap().0, 15);
    /// ```
    #[inline]
...
///     .build();
///
/// let filtered_entity: FilteredEntityRef = query.query(&mut world).single_inner().unwrap();
...
///     .build();
///
/// let (_, filtered_entity) = query.query(&mut world).single_inner().unwrap();
...
///     .build();
///
/// let mut filtered_entity: FilteredEntityMut = query.query_mut(&mut world).single_inner().unwrap();
...
///     .build();
///
/// let (_, mut filtered_entity) = query.query_mut(&mut world).single_inner().unwrap();
...
mod tests {
    ...
    fn entity_mut_insert_by_id() {
        ...
        let components: Vec<_> = world.query_mut::<&TestComponent>().into_iter().collect();
    ...
    fn entity_mut_insert_bundle_by_id() {
        ...
        let dynamic_components: Vec<_> = world
            .query_mut::<(&TestComponent, &TestComponent2)>()
            .into_iter()
            .collect();
    ...
    fn entity_ref_except() {
        ...
        let mut query = world.query_mut::<EntityRefExcept<TestComponent>>();

        let mut found = false;
        for entity_ref in query.iter_mut() {
            found = true;
    ...
    fn entity_mut_except() {
        ...
        let mut query = world.query_mut::<EntityMutExcept<TestComponent>>();

        let mut found = false;
        for mut entity_mut in &mut query {
            found = true;
