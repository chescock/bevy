    ...
    Query<'w, 's, D, F, S>
{
    ...
    fn as_nop(&self) -> Query<'w, '_, NopWorldQuery<D>, F> {
...
mod tests {
    ...
    fn get_many_uniqueness() {
        ...
        let mut query = world.query_mut::<Entity>();
        ...
        assert!(query
            .get_many_mut::<10>(entities.clone().try_into().unwrap())
            .is_ok());

        assert_eq!(
            query.get_many_mut([entities[0], entities[0]]).unwrap_err(),
            QueryEntityError::AliasedMutability(entities[0])
        );

        assert_eq!(
            query
                .get_many_mut([entities[0], entities[1], entities[0]])
                .unwrap_err(),
            QueryEntityError::AliasedMutability(entities[0])
        );

        assert_eq!(
            query.get_many_mut([entities[9], entities[9]]).unwrap_err(),
