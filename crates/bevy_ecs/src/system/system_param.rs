...
use crate::{
    ...
    system::{Query, RunnableSystemMeta, Single, SystemMeta},
    world::{
...
unsafe impl<D: QueryData + 'static, F: QueryFilter + 'static> SystemParam for Query<'_, '_, D, F> {
    ...
    fn init_state(world: &mut World) -> Self::State {
        QueryState::new_uninitialized(world)
    }
...
unsafe impl<'a, D: QueryData + 'static, F: QueryFilter + 'static> SystemParam for Single<'a, D, F> {
    ...
    fn init_access(
        ...
        Query::init_access(state, system_meta, world, system_name);
    }
    ...
    unsafe fn new_archetype(
        ...
        unsafe { Query::new_archetype(state, archetype, archetype_component_access) };
    }
    ...
    for Option<Single<'a, D, F>>
{
    ...
    fn init_access(
        ...
        Single::init_access(state, system_meta, world, system_name)
    }
    ...
    unsafe fn new_archetype(
        ...
        // SAFETY: Delegate to existing `SystemParam` implementations.
        unsafe { Single::new_archetype(state, archetype, archetype_component_access) };
    }
...
unsafe impl<'a, T: Resource> SystemParam for Res<'a, T> {
    ...
    fn init_state(world: &mut World) -> Self::State {
        world.components.register_resource::<T>()
    }
...
unsafe impl<'a, T: Resource> SystemParam for Option<Res<'a, T>> {
    ...
    fn init_access(
        ...
        Res::<T>::init_access(state, system_meta, world, system_name)
    }
    ...
    fn init_access(
        ...
        if combined_access.has_resource_write(component_id) {
            panic!(
                ...
                core::any::type_name::<T>(), system_name);
...
unsafe impl<'a, T: Resource> SystemParam for Option<ResMut<'a, T>> {
    ...
    fn init_access(
        ...
        ResMut::<T>::init_access(state, system_meta, world, system_name)
    }
...
unsafe impl<'a, T: 'static> SystemParam for NonSend<'a, T> {
    ...
    fn init_state(world: &mut World) -> Self::State {
        world.components.register_non_send::<T>()
    }
...
unsafe impl<T: 'static> SystemParam for Option<NonSend<'_, T>> {
    ...
    fn init_access(
        ...
        NonSend::<T>::init_access(state, system_meta, world, system_name);
    }
...
unsafe impl<'a, T: 'static> SystemParam for Option<NonSendMut<'a, T>> {
    ...
    fn init_access(
        ...
        NonSendMut::<T>::init_access(state, system_meta, world, system_name);
    }
...
unsafe impl<T: SystemParam> SystemParam for Vec<T> {
    ...
    fn init_access(
        ...
        for state in state {
            T::init_access(state, system_meta, world, system_name);
        }
    }
    ...
    fn init_access(
        ...
        let metas = state
            ...
            .map(|state| {
                ...
                T::init_access(state, &mut system_meta.clone(), world, system_name);
                meta
            })
            .collect::<Vec<_>>();
...
macro_rules! impl_system_param_tuple {
    ($(#[$meta:meta])* $($param: ident),*) => {
        ...
        unsafe impl<$($param: SystemParam),*> SystemParam for ($($param,)*) {
            ...
            fn init_access(state: &Self::State, _system_meta: &mut RunnableSystemMeta, _world: &mut World, _system_name: &str) {
                let ($($param,)*) = state;
                $($param::init_access($param, _system_meta, _world, _system_name);)*
            }

            #[inline]
            #[allow(unused_unsafe)]
            unsafe fn new_archetype(($($param,)*): &mut Self::State, _archetype: &Archetype, _archetype_component_access: &mut Access<ArchetypeComponentId>,) {
                // SAFETY: The caller ensures that `archetype` is from the World the state was initialized from in `init_state`.
                unsafe { $($param::new_archetype($param, _archetype, _archetype_component_access);)* }
            }
...
unsafe impl<P: SystemParam + 'static> SystemParam for StaticSystemParam<'_, '_, P> {
    ...
    fn init_access(
        ...
        P::init_access(state, system_meta, world, system_name);
    }
...
impl<T: SystemParam + 'static> DynParamState for ParamState<T> {
    ...
    fn init_access(
        ...
        T::init_access(&self.0, system_meta, world, system_name);
    }
...
unsafe impl SystemParam for DynSystemParam<'_, '_> {
    ...
    fn init_access(
        ...
        state.0.init_access(system_meta, world, system_name);
    }
...
unsafe impl SystemParam for FilteredResources<'_, '_> {
    ...
    fn init_access(
        ...
        if access.has_read_all_resources() {
            ...
            system_meta.archetype_component_access.read_all_resources();
        } else {
...
unsafe impl SystemParam for FilteredResourcesMut<'_, '_> {
    ...
    fn init_access(
        ...
        if access.has_write_all_resources() {
            ...
            system_meta.archetype_component_access.write_all_resources();
        } else {
