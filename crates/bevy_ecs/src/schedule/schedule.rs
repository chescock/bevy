...
impl ScheduleGraph {
    ...
    fn configure_set_inner(
        ...
        if metadata.hierarchy.contains(&set) {
            return Err(ScheduleBuildError::HierarchyLoop(format!("{set:?}")));
        }
        if metadata
            .dependencies
            .iter()
            .any(|dependency| dependency.set == set)
        {
            return Err(ScheduleBuildError::DependencyLoop(format!("{set:?}")));
        }
...
mod tests {
    ...
    use crate::{
        ...
        schedule::{
            tests::ResMut, IntoScheduleConfigs, Schedule, ScheduleBuildError,
            ScheduleBuildSettings, ScheduleConfigs, SystemSet,
        },
        system::Commands,
        world::World,
    };
    ...
    #[test]
    fn set_before_self_fails_with_dependency_loop() {
        let mut schedule = Schedule::new(TestSchedule);
        let ScheduleConfigs::ScheduleConfig(config) =
            TestSet::First.before(TestSet::First).into_configs()
        else {
            panic!()
        };
        let result = schedule.graph.configure_set_inner(config);
        let name = format!("{:?}", TestSet::First);
        assert_eq!(result, Err(ScheduleBuildError::DependencyLoop(name)));
    }

    #[test]
    fn set_in_self_fails_with_hierarchy_loop() {
        let mut schedule = Schedule::new(TestSchedule);
        let ScheduleConfigs::ScheduleConfig(config) =
            TestSet::First.in_set(TestSet::First).into_configs()
        else {
            panic!()
        };
        let result = schedule.graph.configure_set_inner(config);
